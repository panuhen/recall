...
INSTRUCTIONS = (
    ...
    "workspace and follow it. Write results name the `workspace_guide` and may "
    "carry `convention_hints` where a note departs from it. To set up a guide, "
    "create ONE note per workspace with flat frontmatter only (`type: guide`, "
    "`owner`, and `workspace_types` / `workspace_tags` as plain lists, no "
    "nested YAML) and put the conventions in prose in the body; check "
    ...
    "`workspace_health` lists those due for review. "
    ...
    "the note. "
    "Finding notes: `search` matches meaning and keywords and returns a snippet "
    "around the match; `grep` finds an exact string (a name, error code, path or "
    "quote) and returns the matching lines. Check snippets and excerpts before "
    "reading whole notes. "
